...
if not has_telescope then error("This plugin requires telescope.nvim (https://github.com/nvim-telescope/telescope.nvim)") end
...
local mode = { query = 9000, result = 9001 }
...
local function set_config_state(opt_name, value, default) state[opt_name] = value == nil and default or value end
...
local displayer = entry_display.create { items = display_widths, separator = " " }

local make_display = function(entry)
  local display_items = { { entry.result_idx, hl_group.index }, entry.name, { entry.value, hl_group.url } }
  ...
  if state.show_domain_icons then
    ...
    if not state.regenerated_displayer then
      ...
      displayer = entry_display.create { items = display_widths }
...
local function set_finder(new_mode, results)
  ...
  local prompt_text = state.mode == mode.query and ("[Search]" .. state.original_prompt_prefix) or "[Result]" ..
                          state.original_prompt_prefix
  local prompt_hl = state.mode == mode.query and hl_group.prompt_query or hl_group.prompt_default
  state.picker:refresh(new_finder, { reset_prompt = true, new_prefix = { prompt_text, prompt_hl } })
end
...
local function do_search()
  ...
  if not state.anim_timer then state.anim_timer = vim.fn.timer_start(80, in_progress_animation, { ["repeat"] = -1 }) end
...
local function search_or_select(_)
  if state.mode == mode.query then
    ...
    if selection then os.execute(state.open_command .. " \"" .. selection.value .. "\"" .. "&> /dev/null") end
  end
end
...
return telescope.register_extension {
  setup = function(ext_config)
    ...
    if state.show_domain_icons then setup_icon_hl_groups() end
  end,
